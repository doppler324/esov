<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
	...
	//global vars
	global $USER, $relatedFilter, $similarFilter, $servicesFilter, $modelsFilter;
	//other
...
<div id="<?=$this->GetEditAreaId($arResult["ID"]);?>">
	<?$this->SetViewTarget("after_breadcrumb_container");?>
		<h1 class="changeName"><?=$arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"]?></h1>
	<?$this->EndViewTarget();?>
	<div id="catalogElement" class="item<?if(!empty($arResult["SKU_OFFERS"])):?> elementSku<?endif;?>" data-product-iblock-id="<?=$arParams["IBLOCK_ID"]?>" data-from-cache="<?=$arResult["FROM_CACHE"]?>" data-convert-currency="<?=$arParams["CONVERT_CURRENCY"]?>" data-currency-id="<?=$arParams["CURRENCY_ID"]?>" data-hide-not-available="<?=$arParams["HIDE_NOT_AVAILABLE"]?>" data-currency="<?=$arResult["EXTRA_SETTINGS"]["CURRENCY"]?>" data-product-id="<?=!empty($arResult["~ID"]) ? $arResult["~ID"] : $arResult["ID"]?>" data-iblock-id="<?=$arResult["SKU_INFO"]["IBLOCK_ID"]?>" data-prop-id="<?=$arResult["SKU_INFO"]["SKU_PROPERTY_ID"]?>" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-price-code="<?=implode("||", $arParams["PRODUCT_PRICE_CODE"])?>" data-deactivated="<?=$arParams["SHOW_DEACTIVATED"]?>">
		...
		<div id="tableContainer">
			...
			<div id="elementContainer" class="column">
				...
				<?$modelsFilter = array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "PROPERTY_ATT_BRAND" => $arResult["PROPERTIES"]["ATT_BRAND"]["VALUE"], "PROPERTY_MODEL" => $arResult["PROPERTIES"]["MODEL"]["VALUE"], "!ID" => $arResult["ID"]);?>
				<?$APPLICATION->IncludeComponent("dresscode:catalog.section", "squares_models", Array(
						"IBLOCK_TYPE" => "catalog",	// Тип инфоблока
						"IBLOCK_ID" => $arParams["IBLOCK_ID"],	// Инфоблок
						"CONVERT_CURRENCY" => "Y",	// Показывать цены в одной валюте
						"CURRENCY_ID" => $arResult["EXTRA_SETTINGS"]["CURRENCY"],	// Валюта, в которую будут сконвертированы цены
						"ADD_SECTIONS_CHAIN" => "N",	// Включать раздел в цепочку навигации
						"COMPONENT_TEMPLATE" => "squares_models",
						"SECTION_ID" => $arResult["LAST_SECTION"]["ID"],	// ID раздела
						...
						"FILTER_NAME" => "modelsFilter",	// Имя массива со значениями фильтра для фильтрации элементов
						...
						"PAGER_TITLE" => GetMessage("CATALOG_ELEMENT_SIMILAR"),	// Название категорий
				...
				<?$APPLICATION->IncludeComponent("sotbit:seo.meta.tags", "detail", Array(
					"CACHE_GROUPS" => "N",	// Учитывать права доступа
						"CACHE_TIME" => $arParams["CACHE_TIME"],	// Время кеширования (сек.)
						"CACHE_TYPE" => "A",	// Тип кеширования
						"CNT_TAGS" => "30",	// Количество выводимых тегов
						"IBLOCK_ID" => $arResult["PARENT_PRODUCT"]["IBLOCK_ID"],	// Инфоблок
						"IBLOCK_TYPE" => "catalog",	// Тип инфоблока
						"INCLUDE_SUBSECTIONS" => "Y",	// Показывать элементы подразделов раздела
						"SECTION_ID" => $arResult["PARENT_PRODUCT"]["IBLOCK_SECTION_ID"],	// Раздел
						"SORT" => "RANDOM",	// Порядок сортировки тегов
						"SORT_ORDER" => "desc",	// Порядок сортировки
						"COMPONENT_TEMPLATE" => "section",
						"GENERATING_TAGS" => "N",	// Перегенерировать теги
					),
					false
				);?>
				...
				<?/* if($arResult["SHOW_SIMILAR"] == "Y"):?>
		        	<div id="similar">
						...
						<?$similarFilter = array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "PROPERTY_ATT_BRAND" => $arResult["PROPERTIES"]["ATT_BRAND"]["VALUE"], "SECTION_ID" => $arResult["LAST_SECTION"]["ID"],"!PROPERTY_MODEL" => $arResult["PROPERTIES"]["MODEL"]["VALUE"],);?>
						<?$APPLICATION->IncludeComponent("dresscode:catalog.section", "squares_similar", Array(
							"IBLOCK_TYPE" => "catalog",	// Тип инфоблока
								"IBLOCK_ID" => $arParams["IBLOCK_ID"],	// Инфоблок
								"CONVERT_CURRENCY" => "Y",	// Показывать цены в одной валюте
								"CURRENCY_ID" => $arResult["EXTRA_SETTINGS"]["CURRENCY"],	// Валюта, в которую будут сконвертированы цены
								...
								"ELEMENT_SORT_ORDER" => "asc",	// Порядок сортировки элементов
								"ELEMENT_SORT_FIELD2" => "rand",	// Поле для второй сортировки элементов
								"ELEMENT_SORT_ORDER2" => "desc",	// Порядок второй сортировки элементов
