								...
								<?foreach($arItem["VALUES"] as $val => $ar):?><?$i++;?>
									<li class="<?=($i > 5) ? "off" : ""?>">
										...
										/>
											...
											<?=$arItem['ID'] != 52 ? ($arParams['CURRENT_SECTION_CATALOG'] ? $arParams['CURRENT_SECTION_CATALOG']." " : "Часы "). current($arResult['ITEMS'][52]['VALUES'])['VALUE'] . ($arItem["FILTER_HINT"] ? ' '.$arItem["FILTER_HINT"] :'') : ""?> <?=$ar["VALUE"]?>
											<?if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):?>
												<ins class="elCount" data-role="count_<?=$ar["CONTROL_ID"]?>">(<?=$ar["ELEMENT_COUNT"]?>)</ins>
											<?endif;?>
										</label>
